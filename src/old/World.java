...
//		actor.currentRegion = (sector);
		actor.emissiveLightningIntensity = (candelas);
		return actor;
	}
	...
	public boolean checkConsistency() {
		...
		for (int c = 1; c < getTotalSectors(); ++c) {
			...
			if (s.getId() != c) {
				System.out
						.println("id de setor não consistente: "
								+ c + "!=" + s.getId());
				return false;
			}
			...
			for (Direction d : Direction.values()) {
				...
				if (s.getLink(d) >= getTotalSectors() || s.getLink(d) < 0) {
					System.out
							.println("link para setor não existente: s["
									+ c
									+ "].link["
									+ d
									+ "]="
